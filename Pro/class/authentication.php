...
class Authentication_Model extends GoogleAuthenticator
{
    public function getData($username)
    {
        ...
        $user_id = $employee->username_exists($username);
        if(!$user_id) {
            return false;
        }
    ...
    public function putData($username, $data)
    {
        ...
        if(!$employee->updateTokenData($data, $user_id)) {
            return false;
        }
        return true;
    }
...
abstract class GoogleAuthenticator
{
    ...
    public function createURL($user)
    {
        ...
        global $company;
        if($toktype == "hotp") {
            $url = "otpauth://" . $toktype . "/" . $user . "?secret=" . $key . "&counter=" . $counter . "&issuer=" . urlencode("HostFact: " . htmlspecialchars_decode($company->CompanyName));
        } else {
            $url = "otpauth://" . $toktype . "/" . $user . "?secret=" . $key . "&issuer=" . urlencode("HostFact: " . htmlspecialchars_decode($company->CompanyName));
        }
        return $url;
    }
