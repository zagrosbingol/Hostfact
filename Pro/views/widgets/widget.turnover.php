...
if(U_INVOICE_SHOW && U_STATISTICS_SHOW) {
    ...
    $data = $current_widget->getWidgetTurnover($widget["Option1"]);
    $value = $data->SalesExcl;
    if(mb_strlen(money($value, true)) <= 10) {
        $fontsize = "medium";
    } elseif(mb_strlen(money($value, true)) <= 14) {
        $fontsize = "small";
    } else {
        $fontsize = "extrasmall";
    }
    ...
    echo $fontsize;
    echo "\">";
    echo isset($value) ? money($value, true) : money(0, true);
