...
if(U_STATISTICS_SHOW) {
    ...
    $revenue = $current_widget->getWidgetOrders($widget["Option1"]);
    if(mb_strlen(money($revenue["TotalAmountExcl"], true)) <= 10) {
        $fontsize = "medium";
    } elseif(mb_strlen(money($revenue["TotalAmountExcl"], true)) <= 14) {
        $fontsize = "small";
    } else {
        $fontsize = "extrasmall";
    }
    ...
    echo $fontsize;
    echo "\">";
    echo isset($revenue["TotalAmountExcl"]) ? money($revenue["TotalAmountExcl"], true) : money(0, true);
    echo "</h3>\n<font style=\"font-weight:bold;color:#999\">\n\t";
    echo strtolower(__("widget revenue from orders"));
    echo "<br />\n    ";
